// lib/fire_json_full.dart
// Full FireJson implementation (ndjson + indexes + snapshots)
// Public methods added: collectionStream(...) and emitDocChange(...)
...
class FireJson {
  ...
  /// Public API: emit change events (called after append/delete)
  ...
  /// Public API: document-level snapshot stream
  Stream<DocumentSnapshot> docStream(String collection, String id) {
    final docMap = _docControllers.putIfAbsent(collection, () => {});
    // If a controller already exists for this doc, return its stream immediately.
    if (docMap.containsKey(id)) {
      return docMap[id]!.stream;
    }

    // Declare the controller variable first.
    late final StreamController<DocumentSnapshot> ctrl;
    ctrl = StreamController<DocumentSnapshot>.broadcast(
      onListen: () {
        _unawaited(_pushDocSnapshot(collection, id, ctrl));
      },
      onCancel: () {
        // Optionally remove controller to free memory
        // docMap.remove(id);
      },
    );
  ...
  /// Public API: collection-level stream (fires a void event on changes)
...
class DocumentRef {
  ...
  /// Stream of DocumentSnapshot (public)
  Stream<DocumentSnapshot> snapshots() {
    return _fire.docStream(collectionName, id);
  }
}
...
class CollectionRef {
  ...
  Future<void> _openAndForwardCollectionStreamToController(
    FireJson self,
    QueryDescriptor? qd,
    StreamController<QuerySnapshot> target,
  ) async {
    final collectionName = (qd == null)
        ? ''
        : ''; // noop, we will get collection name from context in CollectionRef wrapper
    // Instead of using collectionName here, we'll rely on CollectionRef.snapshots wrapper in UI to call _collectionStream on the specific collection.
    // This helper is retained but not used directly here.
  }
}
